...
import { StarIcon, MapPinIcon, FunnelIcon, MagnifyingGlassIcon, CurrencyRupeeIcon } from '@heroicons/react/24/solid';
...
export default function CategoryPage() {
  ...
  const filteredArtists = useMemo(() => {
    return categoryArtists.filter(artist => {
  ...
  }, [categoryArtists, searchQuery, priceFilter, ratingFilter, locationFilter]);

  const locations = Array.from(new Set(categoryArtists.map(artist => artist.location)));
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-white">
            ...
            >
              ...
              <div className="text-lg">
                <span className="bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full text-gray-800 font-medium">
                  {filteredArtists.length} Expert Artists Available
                </span>
              </div>
            </motion.div>
          </div>
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-gray-700">
                  ...
                  <span className="font-semibold">Search & Filter {category?.name} Artists</span>
                </div>
            ...
            {filteredArtists.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredArtists.map((artist, index) => (
                  <ArtistCard key={artist.id} artist={artist} index={index} categoryId={categoryId} />
                ))}
              </div>
            )}
          </div>
...
function ArtistCard({ artist, index, categoryId }: ArtistCardProps) {
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="flex gap-3">
          ...
          <Link
            href={`/book/${artist.id}?category=${categoryId}`}
